...
module top_level_2(   input clk_100mhz,
    ...
    /*
    // 8 bit Input Audio BRAM (for HPS)
    logic [7:0] HPS_input_aud_bram_data_in;
    logic [7:0] HPS_input_aud_bram_data_out;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_input_aud_bram_addr_in, HPS_input_aud_bram_addr_out;
    assign HPS_input_aud_bram_addr_in = input_aud_bram_addr_in;
    assign HPS_input_aud_bram_data_in = recorder_data[FFT_REAL_BIT_WIDTH-1: FFT_REAL_BIT_WIDTH-8];

    bram_8bit HPS_input_audio_bram (
        .clka(clk_100mhz),    // input wire clka
        .wea(1),      // input wire [0 : 0] wea <-- PORT A ONLY FOR WRITING
        .addra(HPS_input_aud_bram_addr_in),  // input wire [10 : 0] addra
        .dina(HPS_input_aud_bram_data_in),    // input wire [11 : 0] dina
        .clkb(clk_100mhz),    // input wire clkb
        .addrb(HPS_input_aud_bram_addr_out),  // input wire [10 : 0] addrb
        .doutb(HPS_input_aud_bram_data_out)  // output wire [11 : 0] doutb
    );  */
    ...
    /* HPS Frequency Detection */
    /*MemState HPS_input_aud_bram_state;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_input_aud_bram_data_counter;
    logic HPS_input_aud_bram_valid_out, HPS_input_aud_bram_last_out;

    always_ff @( posedge clk_100mhz ) begin 
        if (rst_in) begin
            HPS_input_aud_bram_addr_out <= {ADDRESS_BIT_WIDTH{1'b0}};
        end else if (recorder_last_out && recorder_valid_out) begin
            HPS_input_aud_bram_data_counter <= {ADDRESS_BIT_WIDTH{1'b0}};
            HPS_input_aud_bram_state <= READ_WAIT_INIT_1;
            HPS_input_aud_bram_last_out <= 1'b0;
            HPS_input_aud_bram_valid_out <= 1'b0;
        end else begin
            case (HPS_input_aud_bram_state) 
                READ_WAIT_INIT_1: begin
                    HPS_input_aud_bram_state <= READ_WAIT_INIT_2;
                end
                READ_WAIT_INIT_2: begin
                    HPS_input_aud_bram_state <= ACTION_TO_SLAVE;
                    HPS_input_aud_bram_data_counter <= HPS_input_aud_bram_data_counter + 1'b1;
                    HPS_input_aud_bram_addr_out <= HPS_input_aud_bram_addr_out + 'b1;
                end
                ACTION_TO_SLAVE: begin
                    HPS_input_aud_bram_valid_out <= 1'b1;
                    if (HPS_input_aud_bram_data_counter == 0) begin
                        HPS_input_aud_bram_last_out <= 1'b1;
                        HPS_input_aud_bram_state <= DONE;
                    end else begin 
                        HPS_input_aud_bram_state <= WAITING_FOR_SLAVE;
                    end
                end
                WAITING_FOR_SLAVE: begin
                    if (HPS_fwd_fft_ready_in == 1'b1) begin
                        HPS_input_aud_bram_valid_out <= 1'b0;
                        HPS_input_aud_bram_last_out <= 1'b0;
                        if (HPS_input_aud_bram_data_counter == FFT_DEPTH-1) begin 
                            HPS_input_aud_bram_addr_out <= HPS_input_aud_bram_addr_out + INPUT_WINDOW_SIZE - 1;
                            HPS_input_aud_bram_data_counter <={ADDRESS_BIT_WIDTH{1'b0}}; 
                        end else begin  
                            HPS_input_aud_bram_addr_out <= HPS_input_aud_bram_addr_out + 1'b1;
                            HPS_input_aud_bram_data_counter <= HPS_input_aud_bram_data_counter + 'b1;   
                        end
                        HPS_input_aud_bram_state <= ACTION_TO_SLAVE;
                    end else begin
                        HPS_input_aud_bram_state <= WAITING_FOR_SLAVE;
                ...
                default: HPS_input_aud_bram_state <= READ_WAIT_INIT_1;
            endcase
        end
    end

    logic HPS_fwd_fft_ready_in;
    logic HPS_fwd_fft_valid_out, HPS_fwd_fft_last_out;
    logic [15:0] HPS_fwd_fft_data_out;
                   
    freq_detection_fft fwd_fft2 (.aclk(clk_100mhz), .s_axis_data_tdata(HPS_input_aud_bram_data_out), 
                    .s_axis_data_tvalid(HPS_input_aud_bram_valid_out),
                    .s_axis_data_tlast(HPS_input_aud_bram_last_out), .s_axis_data_tready(HPS_fwd_fft_ready_in),
                    .s_axis_config_tdata(0), 
                    .s_axis_config_tvalid(0),
                    .s_axis_config_tready(),
                    .m_axis_data_tdata(HPS_fwd_fft_data_out), .m_axis_data_tvalid(HPS_fwd_fft_valid_out),
                    .m_axis_data_tlast(HPS_fwd_fft_last_out), .m_axis_data_tready(1));

    always_ff @( posedge clk_100mhz ) begin 
        if (rst_in) begin
            HPS_fwd_fft_bram1_addr_in <= 0;
            HPS_fwd_fft_bram2_addr_in <= 0;
            HPS_fwd_fft_bram3_addr_in <= 0;
            HPS_fwd_fft_bram4_addr_in <= 0;
            HPS_fwd_fft_bram5_addr_in <= 0;
            HPS_fwd_fft_bram6_addr_in <= 0;
        end else if (HPS_fwd_fft_valid_out) begin
            HPS_fwd_fft_bram1_addr_in <= HPS_fwd_fft_last_out ? FFT_DEPTH-1 : HPS_fwd_fft_bram1_addr_in + 1'b1;
            HPS_fwd_fft_bram2_addr_in <= HPS_fwd_fft_last_out ? FFT_DEPTH-1 : HPS_fwd_fft_bram2_addr_in + 1'b1;
            HPS_fwd_fft_bram3_addr_in <= HPS_fwd_fft_last_out ? FFT_DEPTH-1 : HPS_fwd_fft_bram3_addr_in + 1'b1;
            HPS_fwd_fft_bram4_addr_in <= HPS_fwd_fft_last_out ? FFT_DEPTH-1 : HPS_fwd_fft_bram4_addr_in + 1'b1;
            HPS_fwd_fft_bram5_addr_in <= HPS_fwd_fft_last_out ? FFT_DEPTH-1 : HPS_fwd_fft_bram5_addr_in + 1'b1;
            HPS_fwd_fft_bram6_addr_in <= HPS_fwd_fft_last_out ? FFT_DEPTH-1 : HPS_fwd_fft_bram6_addr_in + 1'b1;
            HPS_fwd_fft_bram1_data_in <= HPS_fwd_fft_data_out;
            HPS_fwd_fft_bram2_data_in <= HPS_fwd_fft_data_out;
            HPS_fwd_fft_bram3_data_in <= HPS_fwd_fft_data_out;
            HPS_fwd_fft_bram4_data_in <= HPS_fwd_fft_data_out;
            HPS_fwd_fft_bram5_data_in <= HPS_fwd_fft_data_out;
            HPS_fwd_fft_bram6_data_in <= HPS_fwd_fft_data_out;
        end
    end

    // 8 bit Forward FFT BRAMs (for HPS)
    logic [15:0] HPS_fwd_fft_bram1_data_in;
    logic [15:0] HPS_fwd_fft_bram1_data_out;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram1_addr_in;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram1_addr_out;
    
    bram_16bit HPS_fwd_fft_bram1 (
        .clka(clk_100mhz),    // input wire clka
        .wea(1),      // input wire [0 : 0] wea <-- PORT A ONLY FOR WRITING
        .addra(HPS_fwd_fft_bram1_addr_in),  // input wire [10 : 0] addra
        .dina(HPS_fwd_fft_bram1_data_in),    // input wire [11 : 0] dina
        .clkb(clk_100mhz),    // input wire clkb
        .addrb(HPS_fwd_fft_bram1_addr_out),  // input wire [10 : 0] addrb
        .doutb(HPS_fwd_fft_bram1_data_out)  // output wire [11 : 0] doutb
    );

    logic [15:0] HPS_fwd_fft_bram2_data_in;
    logic [15:0] HPS_fwd_fft_bram2_data_out;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram2_addr_in;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram2_addr_out;
    
    bram_16bit HPS_fwd_fft_bram2 (
        .clka(clk_100mhz),    // input wire clka
        .wea(1),      // input wire [0 : 0] wea <-- PORT A ONLY FOR WRITING
        .addra(HPS_fwd_fft_bram2_addr_in),  // input wire [10 : 0] addra
        .dina(HPS_fwd_fft_bram2_data_in),    // input wire [11 : 0] dina
        .clkb(clk_100mhz),    // input wire clkb
        .addrb(HPS_fwd_fft_bram2_addr_out),  // input wire [10 : 0] addrb
        .doutb(HPS_fwd_fft_bram2_data_out)  // output wire [11 : 0] doutb
    );
    
    logic [15:0] HPS_fwd_fft_bram3_data_in;
    logic [15:0] HPS_fwd_fft_bram3_data_out;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram3_addr_in;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram3_addr_out;
    
    bram_16bit HPS_fwd_fft_bram3 (
        .clka(clk_100mhz),    // input wire clka
        .wea(1),      // input wire [0 : 0] wea <-- PORT A ONLY FOR WRITING
        .addra(HPS_fwd_fft_bram3_addr_in),  // input wire [10 : 0] addra
        .dina(HPS_fwd_fft_bram3_data_in),    // input wire [11 : 0] dina
        .clkb(clk_100mhz),    // input wire clkb
        .addrb(HPS_fwd_fft_bram3_addr_out),  // input wire [10 : 0] addrb
        .doutb(HPS_fwd_fft_bram3_data_out)  // output wire [11 : 0] doutb
    );
    
    logic [15:0] HPS_fwd_fft_bram4_data_in;
    logic [15:0] HPS_fwd_fft_bram4_data_out;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram4_addr_in;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram4_addr_out;
    
    bram_16bit HPS_fwd_fft_bram4 (
        .clka(clk_100mhz),    // input wire clka
        .wea(1),      // input wire [0 : 0] wea <-- PORT A ONLY FOR WRITING
        .addra(HPS_fwd_fft_bram4_addr_in),  // input wire [10 : 0] addra
        .dina(HPS_fwd_fft_bram4_data_in),    // input wire [11 : 0] dina
        .clkb(clk_100mhz),    // input wire clkb
        .addrb(HPS_fwd_fft_bram4_addr_out),  // input wire [10 : 0] addrb
        .doutb(HPS_fwd_fft_bram4_data_out)  // output wire [11 : 0] doutb
    );

    logic [15:0] HPS_fwd_fft_bram5_data_in;
    logic [15:0] HPS_fwd_fft_bram5_data_out;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram5_addr_in;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram5_addr_out;
    
    bram_16bit HPS_fwd_fft_bram5 (
        .clka(clk_100mhz),    // input wire clka
        .wea(1),      // input wire [0 : 0] wea <-- PORT A ONLY FOR WRITING
        .addra(HPS_fwd_fft_bram5_addr_in),  // input wire [10 : 0] addra
        .dina(HPS_fwd_fft_bram5_data_in),    // input wire [11 : 0] dina
        .clkb(clk_100mhz),    // input wire clkb
        .addrb(HPS_fwd_fft_bram5_addr_out),  // input wire [10 : 0] addrb
        .doutb(HPS_fwd_fft_bram5_data_out)  // output wire [11 : 0] doutb
    );

    logic [15:0] HPS_fwd_fft_bram6_data_in;
    logic [15:0] HPS_fwd_fft_bram6_data_out;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram6_addr_in;
    logic [ADDRESS_BIT_WIDTH-1:0] HPS_fwd_fft_bram6_addr_out;
    
    bram_16bit HPS_fwd_fft_bram6 (
        .clka(clk_100mhz),    // input wire clka
        .wea(1),      // input wire [0 : 0] wea <-- PORT A ONLY FOR WRITING
        .addra(HPS_fwd_fft_bram6_addr_in),  // input wire [10 : 0] addra
        .dina(HPS_fwd_fft_bram6_data_in),    // input wire [11 : 0] dina
        .clkb(clk_100mhz),    // input wire clkb
        .addrb(HPS_fwd_fft_bram6_addr_out),  // input wire [10 : 0] addrb
        .doutb(HPS_fwd_fft_bram6_data_out)  // output wire [11 : 0] doutb
    );*/
