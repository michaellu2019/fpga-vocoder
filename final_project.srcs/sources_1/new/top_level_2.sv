...
module top_level_2(   input clk_100mhz,
    ...
    logic sqsum_shifted_valid, sqsum_shifted_last, sqsum_shifted_ready;
    logic [31:0] sqsum_shifted_data;
    
    logic fifo_valid, fifo_last, fifo_ready;
    logic [31:0] fifo_data;
    logic fifo_shifted_valid, fifo_shifted_last, fifo_shifted_ready;
    logic [31:0] fifo_shifted_data;
    
    logic sqrt_valid, sqrt_last;
    logic [23:0] sqrt_data;
    logic sqrt_shifted_valid, sqrt_shifted_last;
    logic [23:0] sqrt_shifted_data;
...
//    assign coeff_decrease[ADDRESS_BIT_WIDTH-1:0] = NFFT_WINDOW_SIZE/1.5;
    ...
    bram_32bit viz_shifted_fft_bram (
        ...
        .clkb(clk_100mhz),    // input wire clkb
        .addrb(fft_bram_addr_out),  // input wire [10 : 0] addrb
        .doutb(fft_bram_data_out)  // output wire [11 : 0] doutb
    );
    
    logic [2*BIT_DEPTH-1:0] sqsum_data_in;
    logic fft_bram_last_out;
    logic fft_bram_valid_out;
    
    logic sqsum_ready_in;                
    MemState sqsum_mem_state;
...
//            fft_bram_addr_out <= fft_bram_addr_out + 'b1;
            case (sqsum_mem_state) 
                ...
                ACTION_TO_SLAVE: begin
                    sqsum_data_in <= fft_bram_data_out;
                    fft_bram_valid_out <= 1'b1;
                    if (fft_bram_addr_out == 0) begin
                        fft_bram_last_out <=1'b1;
                ...
                WAITING_FOR_SLAVE: begin
                    if (sqsum_ready_in == 1'b1) begin
                        fft_bram_valid_out <= 1'b0;
                        if (fft_bram_addr_out == NFFT_WINDOW_SIZE-1) begin
                            fft_bram_addr_out <={ADDRESS_BIT_WIDTH{1'b0}};
                        end else begin
                            fft_bram_addr_out <= fft_bram_addr_out + 'b1;
                        end
...
//                            .m00_axis_tready(sqsum_shifted_ready));
    square_and_sum_v1_0 sq_shifted(.s00_axis_aclk(clk_100mhz), .s00_axis_aresetn(1'b1),
                            .s00_axis_tready(sqsum_ready_in),
                            .s00_axis_tdata(sqsum_data_in),.s00_axis_tlast(fft_bram_last_out),
                            .s00_axis_tvalid(fft_bram_valid_out),.m00_axis_aclk(clk_100mhz),
                            .m00_axis_aresetn(1'b1),. m00_axis_tvalid(sqsum_shifted_valid),
                            .m00_axis_tdata(sqsum_shifted_data),.m00_axis_tlast(sqsum_shifted_last),
                            .m00_axis_tready(sqsum_shifted_ready));
...
//                            .m00_axis_tready(sqsum_shifted_ready));
    ...
    axis_data_fifo_0 fifo_shifted (.s_axis_aclk(clk_100mhz), .s_axis_aresetn(1'b1),
                             .s_axis_tvalid(sqsum_shifted_valid), .s_axis_tready(sqsum_shifted_ready),
                             .s_axis_tdata(sqsum_shifted_data), .s_axis_tlast(sqsum_shifted_last),
                             .m_axis_tvalid(fifo_shifted_valid), .m_axis_tdata(fifo_shifted_data),
                             .m_axis_tready(fifo_shifted_ready), .m_axis_tlast(fifo_shifted_last));   
...
//                    spectrogram_raw_amp_in <= 16'd65000;
    ...
    localparam FFT_WINDOW_SIZE = 2048;
    ...
    value_bram fft3k (.addra(addr_count), .clka(clk_100mhz), .dina({8'b0,sqrt_data}),
                    ...
                    .web(1'b0), .enb(1'b1));
    // testing below //
    /*logic [31:0] fftk_ampl_t;
    logic [31:0] fft2k_ampl_t;
    logic [31:0] fft3k_ampl_t;
    logic [31:0] amp_out_t;
    fft_test_rom display_rom (.addra(draw_addr), .clka(pixel_clk), .douta(amp_out_t));
    fft_test_rom fftk_rom (.addra(fftk_addr), .clka(pixel_clk), .douta(fftk_ampl_t));
    fft_test_rom fft2k_rom (.addra(fftk_addr*2), .clka(pixel_clk), .douta(fft2k_ampl_t));
    fft_test_rom fft3k_rom (.addra(fftk_addr*3), .clka(pixel_clk), .douta(fft3k_ampl_t));  */           
    // testing above                
    ...
    always_ff @(posedge pixel_clk) begin
        if (rst_in == 1'b1) begin
            ...
            hps_s_last <= 1'b0;
            hps_s_valid <= 1'b0; 
        end else if (sqrt_last == 1'b1 && sqrt_valid == 1'b1) begin // kinda reset
            ...
            case (hps_mem_state) 
                ...
                ACTION_TO_SLAVE: begin
                    if (fftk_addr == 1)
                        ...
                        hps_s_data <= {fftk_ampl, fft2k_ampl, fft3k_ampl}; // TODO: change back after testing
                    hps_s_valid <= 1'b1;
    ...
    always_ff @(posedge clk_100mhz) begin
        if (rst_in) begin
            shifted_addr_count <= 0;
        end else if (!rst_in && sqrt_shifted_valid) begin
            if (sqrt_shifted_last) begin
                shifted_addr_count <= 'd0; //allign
            end else begin
                shifted_addr_count <= shifted_addr_count + 1;
            end
        end
    end 
                    
    value_bram mvb_shifted (.addra(shifted_addr_count), .clka(clk_100mhz), .dina({8'b0,sqrt_shifted_data}),
                    .douta(), .ena(1'b1), .wea(sqrt_shifted_valid),.dinb(0),
                    .addrb(shifted_draw_addr), .clkb(pixel_clk), .doutb(shifted_amp_out),
                    .web(1'b0), .enb(1'b1));     
