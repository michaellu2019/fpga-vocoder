...
module top_level(   input clk_100mhz,
    ...
    vga_clk myvga (.clk_in1(clk_100mhz), .clk_out1(pixel_clk)); // 65Mhz
    ...
    logic [9:0] draw_addr;
    logic [9:0] draw_addr_t; // TODO: remove after testing
    logic [31:0] amp_out;
    logic [31:0] amp_out_t; // TODO: remove after testing 
    logic [10:0] hcount;
    logic [9:0] vcount;
    logic       vsync;
    logic       hsync;
    ...
    value_bram mvb (.addra(addr_count), .clka(clk_100mhz), .dina({8'b0,sqrt_data}),
    ...
    ////
    // Testing below   
    logic [9:0] fftk_addr_t;
    logic [31:0] fftk_ampl_t;
    logic [31:0] fft2k_ampl_t;
    logic [31:0] fft3k_ampl_t;
    
    fft_test_rom display_rom (.addra(draw_addr_t), .clka(pixel_clk), .douta(amp_out_t));
    fft_test_rom fftk_rom (.addra(fftk_addr_t), .clka(pixel_clk), .douta(fftk_ampl_t));
    fft_test_rom fft2k_rom (.addra(fftk_addr_t*2), .clka(pixel_clk), .douta(fft2k_ampl_t));
    fft_test_rom fft3k_rom (.addra(fftk_addr_t*3), .clka(pixel_clk), .douta(fft3k_ampl_t));
    // testing above
    ////
...
//        end
        ...
        if ((amp_out>>sw[3:0])>=768-vcount)begin
            if (draw_addr == nat_freq+10'd2) // 2 clock cycles delay between updating address and getting amp_out
                rgb <= 12'hFFF;
            else
                rgb <= sw[15:4];
        end else begin
